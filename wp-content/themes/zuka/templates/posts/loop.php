...
$loop_index         = absint(zuka_get_theme_loop_prop('loop_index', 1));

$is_main_loop       = zuka_get_theme_loop_prop('is_main_loop', false);
$loop_name          = zuka_get_theme_loop_prop('loop_name', false);
$show_thumbnail     = zuka_get_theme_loop_prop('show_thumbnail', true);
$layout             = zuka_get_theme_loop_prop('loop_layout', 'grid');
$style              = zuka_get_theme_loop_prop('loop_style', 1);
$thumbnail_size     = zuka_get_theme_loop_prop('image_size', 'thumbnail');
$title_tag          = zuka_get_theme_loop_prop('title_tag', 'h3');
$excerpt_length     = zuka_get_theme_loop_prop('excerpt_length', 0);
...
if($is_main_loop){
    $height_mode = Zuka()->settings()->get('blog_thumbnail_height_mode', $height_mode);
    $thumb_custom_height = Zuka()->settings()->get('blog_thumbnail_height_custom', $thumb_custom_height);
}
...
if (!has_post_thumbnail() || ($is_main_loop && !$show_thumbnail)) {
    ...
    if($thumbnail_obj = Zuka()->images()->get_attachment_image_src( get_post_thumbnail_id(), $thumbnail_size )){
        ...
        <div class="loop__item__inner2">
            ...
            <div class="loop__item__thumbnail">
                ...
                if('gallery' == get_post_format()){
                    $galleries = zuka_get_image_for_post_type_gallery(get_the_ID(), $thumbnail_size);
                    $gallery_html = '';
                ...
                ><?php
                    if('gallery' != get_post_format() && has_post_thumbnail()) {
                        if ( has_post_thumbnail() ) {
                            echo Zuka()->images()->render_image( $thumb_src, array(
                                'width' => $thumb_width,
                                'height' => $thumb_height,
                                'alt' => get_the_title()
                            ) );
                ...
                if('quote' == get_post_format()){
                    zuka_get_image_for_post_type_quote(get_the_ID());
                }
                ...
                <div class="loop__item__info2">
                    <div class="loop__item__meta loop__item__meta__top"><?php
                        ...
                        if($style == 'echeck'){
                            ...
                            if( ($layout == 'grid' && ($style != 2 && $style != 4 && $style != 6 && $style != 'slider' && $style != 5)) || ( $layout == 'list' && $style == 1 )){
                        ...
                        if( 
                            ($layout == 'grid' && ($style == 5 || $style == 4 || $style == 6 || $style == 'slider'))
                            || 
                            ($layout == 'list' && ($style == 2 || $style == 3 ))
                        ){
                            zuka_entry_meta_item_category_list('<div class="only__term_links loop__item__meta--item loop__item__termlink blog_item--category-link">', '</div>', '');
                        }
                    ...
                    if($style == 'echeck'){
                        ...
                        zuka_entry_meta_item_author(false);
                        zuka_entry_meta_item_postdate();
                        echo '</div>';
                    }
                    else{
                        if( ($layout == 'grid' && ($style == 2 || $style == 4 || $style == 6 || $style == 5)) || ( $layout == 'list' && ($style == 3 || $style == 2)) ){
                            echo '<div class="loop__item__meta loop__item__meta__middle">';
                                if($layout == 'grid' && $style == 5){
                                    echo sprintf(
                                        '<div class="loop__item__meta--item posted-on"><a href="%1$s"><span class="day">%2$s</span><span class="month">%3$s</span></a></div>',
                                        esc_url(get_the_permalink()),
                                        get_the_date('d'),
                                        get_the_date('M')
                                    );
                        ...
                        <div class="loop__item__desc">
                            <?php
                                if($layout == 'grid' && $style == 4){
                                    ...
                                    echo zuka_get_the_excerpt( $loop_name == 'related_posts' ? $excerpt_length : null);
                                }
                            ?>
                        </div>
                    <?php endif; ?>
                    <?php
                            ...
                            printf(
                                ...
                                get_the_permalink(),
                                esc_html_x('Read more', 'front-view', 'zuka'),
...
if($layout == 'list' && $style == 1){
    ...
    $loop_index++;
    zuka_set_theme_loop_prop('loop_index', $loop_index);
}