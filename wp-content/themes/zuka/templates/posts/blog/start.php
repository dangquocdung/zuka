...
$blog_item_space = Zuka()->settings()->get('blog_item_space', 'default');

if($blog_item_space == 'zero'){
    $blog_item_space = 0;
}

$blog_design = Zuka()->settings()->get('blog_design', 'grid_4');
$blog_columns = wp_parse_args( (array) Zuka()->settings()->get('blog_post_column'), array('lg'=> 1,'md'=> 1,'sm'=> 1,'xs'=> 1, 'mb' => 1) );
$blog_masonry = ( Zuka()->settings()->get('blog_masonry') == 'on' ) ? true : false;
$blog_pagination_type = Zuka()->settings()->get('blog_pagination_type', 'pagination');
...
if($layout == 'grid'){
    ...
    $css_classes[] = zuka_render_grid_css_class_from_columns($blog_columns);
}
...
$thumbnail_size     = Zuka_Helper::get_image_size_from_string(Zuka()->settings()->get('blog_thumbnail_size', 'full'), 'full');
$excerpt_length     = Zuka()->settings()->get('blog_excerpt_length');
$content_type       = (Zuka()->settings()->get('blog_content_display', 'excerpt') == 'excerpt') ? 'excerpt' : 'full';
$show_thumbnail     = (Zuka()->settings()->get('featured_images_blog') == 'on') ? true : false;
$height_mode        = 'original';
$thumb_custom_height= '';

zuka_set_theme_loop_prop('is_main_loop', true, true);
zuka_set_theme_loop_prop('loop_layout', $layout);
zuka_set_theme_loop_prop('loop_style', $style);
zuka_set_theme_loop_prop('title_tag', 'h2');
zuka_set_theme_loop_prop('image_size', $thumbnail_size);
zuka_set_theme_loop_prop('excerpt_length', $excerpt_length);
zuka_set_theme_loop_prop('content_type', $content_type);
zuka_set_theme_loop_prop('show_thumbnail', $show_thumbnail);
zuka_set_theme_loop_prop('height_mode', $height_mode);
zuka_set_theme_loop_prop('height', $thumb_custom_height);
zuka_set_theme_loop_prop('responsive_column', $blog_columns);
