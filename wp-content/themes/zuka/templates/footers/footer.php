...
$footer_layout = Zuka()->layout()->get_footer_layout();
$number_col = absint(substr(ltrim($footer_layout),0,1));

$footer_copyright = Zuka()->settings()->get('footer_copyright');
$enable_footer_top = Zuka()->settings()->get('enable_footer_top');
...
<footer id="colophon" class="site-footer la-footer-<?php echo esc_attr($footer_layout)?>">
    <?php if(zuka_string_to_bool($enable_footer_top)): ?>
    <div class="footer-top-area">
        <div class="container">
            <div class="row">
                <?php
                dynamic_sidebar( apply_filters('zuka/filter/footer_top_area', 'footer_top_area'));
                ?>
            </div>
        </div>
    </div>
        ...
        <div class="container">
            <div class="row">
                ...
                for ( $i = 1; $i <= $number_col; $i++ ){
                    ...
                    dynamic_sidebar( apply_filters('zuka/filter/footer_column_'. $i, 'f-col-'. $i, $footer_layout));
                    echo '</div></div>';
                }
                ?>
            </div>
    ...
    <?php if(Zuka()->settings()->get('enable_footer_copyright','no') == 'yes' && !empty($footer_copyright)): ?>
        <div class="footer-bottom">
            <div class="container">
                <div class="footer-bottom-inner">
                    <?php echo Zuka_Helper::remove_js_autop( $footer_copyright );?>
                </div>
            </div>
        </div>
    <?php endif; ?>
