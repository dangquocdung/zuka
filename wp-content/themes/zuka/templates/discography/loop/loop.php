...
$thumbnail_size = zuka_get_theme_loop_prop('image_size', 'full');
$title_tag      = zuka_get_theme_loop_prop('title_tag', 'h5');
$post_class     = array('loop__item','grid-item','release__item');

$release_itunes = Zuka()->settings()->get_post_meta( get_the_ID(), '', '_lastudio_release_itunes' );
$release_gplay  = Zuka()->settings()->get_post_meta( get_the_ID(), '', '_lastudio_release_google_play' );
$release_amazon = Zuka()->settings()->get_post_meta( get_the_ID(), '', '_lastudio_release_amazon' );
$release_buy    = Zuka()->settings()->get_post_meta( get_the_ID(), '', '_lastudio_release_buy' );
$release_free   = Zuka()->settings()->get_post_meta( get_the_ID(), '', '_lastudio_release_free' );
        ...
        <div class="loop__item__inner2">
            ...
            <div class="loop__item__thumbnail">
                <div class="loop__item__thumbnail--bkg<?php echo esc_attr($thumb_css_class); ?>"
                    ...
                    <?php echo Zuka()->images()->render_image($thumb_src, array('width' => $thumb_width, 'height' => $thumb_height, 'alt' => get_the_title())); ?>
                </div>
                <div class="release__item__applink">
                    ...
                    if(!empty($release_itunes)){
                        echo sprintf(
                            ...
                            esc_attr_x('Buy on iTunes', 'front-end', 'zuka')
                        );
                    }
                    if(!empty($release_gplay)){
                        echo sprintf(
                            ...
                            esc_attr_x('Buy on Google Play', 'front-end', 'zuka')
                        );
                    }
                    ?>
                </div>
