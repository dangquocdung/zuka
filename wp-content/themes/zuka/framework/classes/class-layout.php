...
class Zuka_Layout {
    ...
    public function __construct() {
        ...
        add_action( 'zuka/action/before_render_body', array( $this, 'render_pageloader_icon' ), 1);
        add_action( 'zuka/action/before_render_main', array( $this, 'render_additional_block_content_top' ) );
        add_action( 'zuka/action/before_render_main_inner', array( $this, 'render_additional_block_content_inner_top' ) );
        add_action( 'zuka/action/after_render_main_inner', array( $this, 'render_additional_block_content_inner_bottom' ) );
        add_action( 'zuka/action/after_render_main', array( $this, 'render_additional_block_content_bottom' ) );

        add_filter('zuka/filter/sidebar_primary_name', array( $this, 'set_sidebar_name'), 10 );
        add_filter('zuka/filter/main_menu_location', array( $this, 'main_menu_location'), 10 );

        add_action('wp_head', array( $this, 'render_favicon') );
        add_action('admin_head', array( $this, 'render_favicon') );

        add_filter('zuka/get_site_layout', array( $this, 'get_404_layout') );

        add_action('zuka/action/after_render_body', array( $this, 'render_footer_bottom' ) );
    }
    ...
    public function body_classes( $classes ) {
        ...
        $main_fullwidth             = Zuka()->settings()->get_setting_by_context('main_full_width','no');
        $header_full_width          = Zuka()->settings()->get_setting_by_context('header_full_width', 'no');
        $header_sticky              = Zuka()->settings()->get_setting_by_context('header_sticky', 'no');
        $header_transparency        = Zuka()->settings()->get_setting_by_context('header_transparency', 'no');

        $footer_full_width          = Zuka()->settings()->get_setting_by_context('footer_full_width','no');

        $body_boxed                 = Zuka()->settings()->get('body_boxed', 'no');
        ...
        if(Zuka()->settings()->get('page_loading_animation', 'off') == 'on'){
        ...
        if($site_layout == 'col-1c'){
            $blog_small_layout = Zuka()->settings()->get('blog_small_layout', 'off');
            if(is_singular('post')){
                $single_small_layout_global = Zuka()->settings()->get('single_small_layout', 'off');
                $single_small_layout = Zuka()->settings()->get_post_meta( get_queried_object_id() , 'small_layout' );
            ...
            if(in_array('is_category', $context) || in_array('is_tag', $context)){
                $blog_archive_small_layout = Zuka()->settings()->get_term_meta( get_queried_object_id() , 'small_layout' );
    ...
    public function get_site_layout(){
        $layout = Zuka()->settings()->get_setting_by_context('layout', 'col-1c');
        return apply_filters('zuka/get_site_layout', $layout);
    }
    ...
    public function get_main_content_css_class( $el_class =  '' ){
        ...
        if($site_layout == 'col-1c'){
            ...
            if(is_tag() || is_category()){
                $blog_archive_small_layout = Zuka()->settings()->get_post_meta( get_queried_object_id() , 'small_layout' );
                if($blog_archive_small_layout == 'on'){
                    $_class = 'col-md-10 col-md-offset-1';
                }
                else{
    ...
    public function get_header_layout(){
        return Zuka()->settings()->get_setting_by_context('header_layout', 1);
    }

    public function get_page_title_bar_layout(){
        return Zuka()->settings()->get_setting_by_context('page_title_bar_layout', 'hide');
    }

    public function get_footer_layout(){
        return Zuka()->settings()->get_setting_by_context('footer_layout', '1col');
    }

    public function render_logo( $default_logo = null ){
        $logo = Zuka()->settings()->get('logo', false);
        $logo2x = Zuka()->settings()->get('logo_2x', false);
        if(empty($default_logo)){
            $default_logo = apply_filters('zuka/setting/logo/default', Zuka::$template_dir_url . '/assets/images/logo.png');
        }
    ...
    public function render_transparency_logo( ){
        $logo = Zuka()->settings()->get('logo_transparency', false);
        $logo2x = Zuka()->settings()->get('logo_transparency_2x', false);
        $default_logo = apply_filters('zuka/setting/logo/transparency', Zuka::$template_dir_url . '/assets/images/logo.png');
    ...
    public function render_mobile_logo(){
        $logo = Zuka()->settings()->get('logo_mobile', false);
        $logo2x = Zuka()->settings()->get('logo_mobile_2x', false);
        $logo_src = $default_logo = apply_filters('zuka/setting/logo_mobile/default', Zuka::$template_dir_url . '/assets/images/logo.png');
        $logo_2x_src = false;
        if($logo){
            $logo_src = wp_get_attachment_image_url( $logo, 'full' );
        }
    ...
    public function render_mobile_transparency_logo(){
        $logo = Zuka()->settings()->get('logo_mobile_transparency', false);
        $logo2x = Zuka()->settings()->get('logo_mobile_transparency_2x', false);
        $logo_src = $default_logo = apply_filters('zuka/setting/logo_mobile/transparency', Zuka::$template_dir_url . '/assets/images/logo.png');
        $logo_2x_src = false;
        if($logo){
            $logo_src = wp_get_attachment_image_url( $logo, 'full' );
        }
    ...
    public function render_main_nav( $args = array() ) {
        $default = array(
            ...
            'link_after'    => '</span>',
            'fallback_cb'   => array( 'Zuka_MegaMenu_Walker', 'fallback' ),
            'walker'        => new Zuka_MegaMenu_Walker
        );

        $menu_args = array_merge( $default, apply_filters( 'zuka/filter/main_menu_location' , array(
    ...
    public function render_header_tpl(){
        if(Zuka()->settings()->get_setting_by_context('hide_header') == 'yes'){
            return;
        }
    ...
    public function render_header_mobile_tpl(){
        ...
        $value = Zuka()->settings()->get('header_mb_layout', '1');
    ...
    public function render_footer_tpl(){
        if(Zuka()->settings()->get_setting_by_context('hide_footer') == 'yes'){
            return;
        }
    ...
    public function set_sidebar_name( $sidebar ){
        ...
        if(in_array( 'is_search', $context)){
            if( ($sidebar_search = Zuka()->settings()->get('search_sidebar', $sidebar)) && !empty( $sidebar_search) ) {
        ...
        if(in_array('is_category', $context) || in_array( 'is_tag', $context )){
            ...
            $sidebar = Zuka()->settings()->get('blog_archive_sidebar', $sidebar);

            if( Zuka()->settings()->get('blog_archive_global_sidebar', false) ){
                /*
            ...
            $_sidebar = Zuka()->settings()->get_term_meta( get_queried_object_id(), 'sidebar');
        ...
        if(is_singular('post')){
            $sidebar = Zuka()->settings()->get('posts_sidebar', $sidebar);
            if( Zuka()->settings()->get('posts_global_sidebar', false) ){
                /*
                 * Return global sidebar if option will be enable
                 * We don't need more checking in context
                 */
            ...
            $_sidebar = Zuka()->settings()->get_post_meta( get_queried_object_id(), 'sidebar');
            if(!empty($_sidebar)){
                return $_sidebar;
            }
        }
        ...
        if(in_array('is_tax', $context) && is_tax(get_object_taxonomies( 'la_portfolio' ))){
            $sidebar = Zuka()->settings()->get('portfolio_archive_sidebar', $sidebar);
            if( Zuka()->settings()->get('portfolio_archive_global_sidebar', false) ){
                /*
                 * Return global sidebar if option will be enable
                 * We don't need more checking in context
                 */
        ...
        if(is_singular('la_portfolio')){
            $sidebar = Zuka()->settings()->get('portfolio_sidebar', $sidebar);
            if( Zuka()->settings()->get('portfolio_global_sidebar', false) ){
                /*
                 * Return global sidebar if option will be enable
                 * We don't need more checking in context
                 */
        ...
        if(is_page()){
            $sidebar = Zuka()->settings()->get('pages_sidebar', $sidebar);
            if( Zuka()->settings()->get('pages_global_sidebar', false) ){
                /*
                 * Return global sidebar if option will be enable
                 * We don't need more checking in context
                 */
    ...
    public function main_menu_location( $args ){
        if( $menu_id = Zuka()->settings()->get_setting_by_context('main_menu') ){
    ...
    public function render_additional_block_content_top(){
        if( $block_id = (int) Zuka()->settings()->get_setting_by_context('block_content_top') ){
    ...
    public function render_additional_block_content_inner_top(){
        if( $block_id = (int) Zuka()->settings()->get_setting_by_context('block_content_inner_top') ){
    ...
    public function render_additional_block_content_bottom(){
        if( $block_id = (int) Zuka()->settings()->get_setting_by_context('block_content_bottom') ){
    ...
    public function render_additional_block_content_inner_bottom(){
        if( $block_id = (int) Zuka()->settings()->get_setting_by_context('block_content_inner_bottom') ){
    ...
    public function render_pageloader_icon(){
        if(Zuka()->settings()->get('page_loading_animation', 'off') == 'on'){
            $loading_style = Zuka()->settings()->get('page_loading_style', '1');
            if($loading_style == 'custom'){
                if($img = Zuka()->settings()->get('page_loading_custom')){
    ...
    public function render_favicon(){
        if ( ! function_exists( 'has_site_icon' ) || ! has_site_icon() ) {
            if( $favicon = wp_get_attachment_image_url(Zuka()->settings()->get('favicon'), 'full') ){
                printf('<link rel="apple-touch-icon" sizes="16x16" href="%s"/>', esc_url($favicon));
            }
            if( $favicon = wp_get_attachment_image_url(Zuka()->settings()->get('favicon_iphone'), 'full') ){
                printf('<link rel="apple-touch-icon" sizes="57x57" href="%s"/>', esc_url($favicon));
            }
            if( $favicon = wp_get_attachment_image_url(Zuka()->settings()->get('favicon_ipad'), 'full') ){
