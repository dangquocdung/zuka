...
class Zuka_Breadcrumbs {
	...
	function __construct( $args = array() ) {
		...
		$defaults = array(
			...
			'labels'            => array(),
			'date_labels'       => array(),
			'action'            => 'zuka/action/breadcrumbs/render_html',
			'css_namespace'     => array(),
			'path_type'         => 'default', // minified , full, default
			'post_taxonomy'     => apply_filters(
				'zuka/filter/breadcrumbs/trail_taxonomies',
				array(
					'post'      => 'category',
					'product'	=> 'product_cat'
				)
		...
		$this->args = apply_filters( 'zuka/filter/breadcrumbs/args', wp_parse_args( $args, $defaults ) );
	...
	public function get_trail() {
		...
		$wrapper_classes = apply_filters( 'zuka/filter/breadcrumbs/wrapper_classes', $wrapper_classes );
		...
		do_action('zuka/action/breadcrumbs/before_render_html', $this, $this->args);

		if ( true === $this->args['echo'] ) {
			echo zuka_render_variable($result);
		}
		else {
			return $result;
		}
	...
	public function get_items() {
		...
		$breadcrumb = apply_filters( 'zuka/filter/breadcrumbs/trail', $breadcrumb, $this->args );
	...
	public function default_labels() {
		...
		$labels = array(
			'browse'              => esc_html_x( 'Browse:', 'front-view', 'zuka' ),
			'home'                => $this->home_title(),
			'error_404'           => esc_html_x( '404 Not Found', 'front-view', 'zuka' ),
			'archives'            => esc_html_x( 'Archives', 'front-view', 'zuka' ),
			'search'              => esc_html_x( 'Search results for &#8220;%s&#8221;', 'front-view', 'zuka' ),
			'paged'               => esc_html_x( 'Page %s', 'front-view', 'zuka' ),
			'archive_minute'      => esc_html_x( 'Minute %s', 'front-view', 'zuka' ),
			'archive_week'        => esc_html_x( 'Week %s', 'front-view', 'zuka' ),
	...
	public function home_title() {
		...
		$title            = esc_html_x( 'Home', 'front-view', 'zuka' );
		$use_custom_title = apply_filters( 'zuka/filter/breadcrumbs/custom_home_title', true );
	...
	public function build_trail() {
		...
		$custom_trail = apply_filters( 'zuka/filter/breadcrumbs/custom_trail', false, $this->args );
		 ...
		 */
		$this->items = apply_filters( 'zuka/filter/breadcrumbs/items', $this->items, $this->args, $this );
	...
	public function add_post_terms( $post_id ) {
		...
		if ( ! empty( $this->args['post_taxonomy'][ $post_type ] ) ) {
			add_filter('get_object_terms', 'zuka_exclude_demo_term_in_category');
			$post_terms = wp_get_post_terms( $post_id, $this->args['post_taxonomy'][ $post_type ] );
			zuka_deactive_filter('get_object_terms', 'zuka_exclude_demo_term_in_category');
	...
	public function prepare_label( $label, $fallback = false ) {
		...
		if ( false !== $fallback ) {
			$fallback = sprintf(
				apply_filters( 'zuka/filter/breadcrumbs/icon_fallback', '<span class="hidden">%s</span>', $fallback ),
				esc_attr( $fallback )
			);
		}
		...
		// Return default icon
		return sprintf(
			apply_filters( 'zuka/filter/breadcrumbs/default_icon_format', '<span class="%1$s">%2$s</span>', $label, $fallback ),
	...
	public function get_breadcrumb() {
		return apply_filters( 'zuka/filter/breadcrumbs/get_breadcrumb', $this->crumbs, $this );
	}
}