...
class Zuka_WooCommerce_Compare {
    ...
    public function remove_item_from_url(){
        if(isset($_GET['la_helpers_compare_remove']) && ( $product_id = absint($_GET['la_helpers_compare_remove'])) ) {
            ...
            if(($key = array_search($product_id, $product_exists)) !== false) {
                ...
                wc_add_notice( sprintf( __('"%1$s" has been removed from list', 'zuka'), get_the_title($product_id) ) );
            }
        }
    }
    ...
    public function ajax_process(){
        ...
        if(apply_filters('zuka/check_ajax_referer', false, 'compare_nonce', 'security')){
    ...
    private function add( $post_id = 0 ){
        $lists = $this->get_data();
        $response = array();
        $response['compare_url'] = zuka_get_compare_url();
        if(in_array($post_id, $lists)){
            $response['message'] = __('Product already in list', 'zuka');
    ...
    private function remove( $post_id = 0 ){
        ...
        if(($key = array_search($post_id, $lists)) !== false) {
            $response['message'] = __('Product has been removed from list', 'zuka');
            $response['has_error'] = false;
            unset($lists[$key]);
            $this->set_lists_for_user($lists);
            $this->set_lists_for_cookie($lists);
            ...
            $response['message'] = __('Product does not exist in list', 'zuka');
            $response['has_error'] = true;
        }
        return count($lists);
    }
