...
class Zuka {
    ...
    private static $original_option_name = 'zuka_options';
    ...
    private function __construct() {
        ...
        wp_cache_add_non_persistent_groups( 'zuka' );
        ...
        $this->settings       = new Zuka_Setting();
        $this->images         = new Zuka_Images();
        $this->layout         = new Zuka_Layout();
        $this->options        = new Zuka_Options();
        $this->favorite       = new Zuka_FavoritePosts();

        new Zuka_Init();
        new Zuka_Blog();
        new Zuka_Widget_CssClass();
    ...
    public function build_crumbs(){
        ...
        if( $this->settings()->get('google_rich_snippets', 'no') == 'yes' ){
            if(function_exists('wc') && is_object(wc()->structured_data)){
                add_action('zuka/action/breadcrumbs/before_render_html', array(wc()->structured_data, 'generate_breadcrumblist_data'), 10);
            }
        }
        $this->breadcrumbs = new Zuka_Breadcrumbs( $args );
    }

    public function set_is_updating() {
        if ( ! self::$is_updating && $_GET && isset( $_GET['zuka_update'] ) && '1' == $_GET['zuka_update'] ) {
    ...
    public static function multilingual_options() {
        // Set the self::$lang.
        if ( ! in_array( Zuka_Multilingual::get_active_language(), array( '', 'en', 'all' ) ) ) {
            self::$lang = '_' . Zuka_Multilingual::get_active_language();
        }
