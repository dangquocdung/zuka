...
class Zuka_Images {
    ...
    public function before_resize(){
        if ( class_exists( 'Jetpack' ) && Jetpack::is_module_active( 'photon' ) ) { // check that we are, in fact, using Photon in the first place
            zuka_deactive_filter( 'image_downsize', array( Jetpack_Photon::instance(), 'filter_image_downsize' ) );
        }
        add_filter('image_downsize', array( &$this, 'image_downsize' ), 10, 3 );
    }

    public function after_resize(){
        zuka_deactive_filter('image_downsize', array( &$this, 'image_downsize' ), 10 );
    ...
    public function image_downsize( $downsize = false , $id, $size ) {
        ...
        $crop = apply_filters('zuka/filter/image_helper/crop', $crop, $id, $size);
    ...
    public function the_post_thumbnail($post = null, $size = 'thumbnail', $attr = '' ){
        echo zuka_render_variable($this->get_post_thumbnail($post, $size, $attr));
    }
