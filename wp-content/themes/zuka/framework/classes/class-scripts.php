...
class Zuka_Scripts
{
    ...
    public function __construct()
    {
        ...
        add_action('zuka/action/head', array( $this, 'get_custom_css_from_setting'));
        add_action('zuka/action/head', array( $this, 'add_custom_header_js' ), 100 );
        add_action('wp_footer', array( $this, 'add_custom_footer_js' ), 100 );

        add_action('plugins_loaded', array( $this, 'override_style_from_builder' ), 20);
    }
    ...
    public function enqueue_scripts()
    {
        ...
        $script_min_path = apply_filters('zuka/filter/js_load_min_file', 'min/');
        ...
        wp_enqueue_style('font-awesome', Zuka::$template_dir_url . '/assets/css/font-awesome.min.css', array(), $theme_version);
        wp_enqueue_style('animate-css', Zuka::$template_dir_url . '/assets/css/animate.min.css', array(), $theme_version);
        wp_enqueue_style('zuka-theme', get_template_directory_uri() . '/style.css', array('font-awesome'), $theme_version);
        ...
        $font_source = Zuka()->settings()->get('font_source', 1);
        switch ($font_source) {
            case '1':
                wp_enqueue_style('zuka-google_fonts', $this->get_google_font_url(), array(), null);
                break;
            case '2':
                wp_enqueue_style('zuka-font_google_code', $this->get_google_font_code_url(), array(), null);
                break;
            case '3':
                wp_enqueue_script('zuka-font_typekit', $this->get_google_font_typekit_url(), array(), null);
                wp_add_inline_script( 'zuka-font_typekit', 'try{ Typekit.load({ async: true }) }catch(e){}' );
                break;
        }

        wp_enqueue_script( 'respond', Zuka::$template_dir_url . '/assets/js/enqueue/min/respond.js');
        ...
        wp_register_script( 'zuka-modernizr-custom', Zuka::$template_dir_url . '/assets/js/enqueue/min/modernizr-custom.js', array('jquery'), $theme_version, true);

        $fullpage_config = array();
        $js_require = array('jquery','zuka-modernizr-custom');

        if (in_array('is_page', Zuka()->get_current_context())) {
            $fp_metadata = Zuka()->settings()->get_post_meta(get_the_ID());

            if (Zuka()->layout()->get_site_layout() == 'col-1c' && (!empty($fp_metadata['enable_fp']) && $fp_metadata['enable_fp'] == 'yes')) {

                wp_enqueue_style('zuka-fullpage', Zuka::$template_dir_url . '/assets/css/fullpage.min.css', array('zuka-theme'), $theme_version);
                ...
                if(!empty($fp_metadata['fp_navigation']) && $fp_metadata['fp_navigation'] != 'off'){
                    if($section_nav_file != 'number'){
                        wp_enqueue_style('zuka-fullpage-nav', Zuka::$template_dir_url . '/assets/css/fullpage/nav/section/'.$section_nav_file.'.css', array('zuka-theme'), $theme_version);
                    }
                }
                if(!empty($fp_metadata['fp_slidenavigation']) && $fp_metadata['fp_slidenavigation'] != 'off'){
                    ...
                    if($slide_nav_file != 'number'){
                        wp_enqueue_style('zuka-fullpage-nav', Zuka::$template_dir_url . '/assets/css/fullpage/nav/slide/'.$slide_nav_file.'.css', array('zuka-theme'), $theme_version);
                    }
                }
                ...
                if (substr($fp_easing, 0, 3) == 'js_') {
                    wp_register_script('zuka-easings', Zuka::$template_dir_url . '/assets/js/enqueue/min/jquery.easings.js', array('jquery'), $theme_version, true);
                    $fullpage_js_require[] = 'zuka-easings';
                }
                if ($fp_scrolloverflow == 'yes') {
                    wp_register_script('zuka-scrolloverflow', Zuka::$template_dir_url . '/assets/js/enqueue/min/jquery.scrolloverflow.js', array('jquery'), $theme_version, true);
                    $fullpage_js_require[] = 'zuka-scrolloverflow';
                }
                wp_register_script('zuka-fullpage-parallax', Zuka::$template_dir_url . '/assets/js/enqueue/min/jquery.fullpage.parallax.js', array('jquery'), $theme_version, true);
                $fullpage_js_require[] = 'zuka-fullpage-parallax';

                wp_register_script('zuka-fullpage', Zuka::$template_dir_url . '/assets/js/enqueue/min/jquery.fullpage.extensions.js', $fullpage_js_require, $theme_version, true);
                $js_require[] = 'zuka-fullpage';

                $fullpage_config = $this->get_fullpage_config();
            }
        }

        if(apply_filters('zuka/filter/force_enqueue_js_external', true)){
            wp_register_script('zuka-plugins', Zuka::$template_dir_url . '/assets/js/plugins/min/plugins-full.js', array('jquery'), $theme_version, true);
            $js_require[] = 'zuka-plugins';
        }

        //wp_enqueue_script('zuka-theme', Zuka::$template_dir_url . '/assets/js/'.$script_min_path.'app.js', $js_require, $theme_version, true);

        wp_enqueue_script('zuka-theme', Zuka::$template_dir_url . '/assets/js/app.js', $js_require, $theme_version, true);

        wp_localize_script('zuka-theme', 'la_theme_config', apply_filters('zuka/filter/global_message_js', array(
            'security' => array(
                ...
                'instagram_token' => esc_attr(Zuka()->settings()->get('instagram_token'))
            ),
            'fullpage' => $fullpage_config,
            'product_single_design' => esc_attr(Zuka()->settings()->get('woocommerce_product_page_design', 1)),
            'product_gallery_column' => esc_attr(json_encode(Zuka()->settings()->get('product_gallery_column', array(
                'xlg'	=> 3,
                'lg' 	=> 3,
                'md' 	=> 3,
                'sm' 	=> 5,
            ...
            'single_ajax_add_cart' => esc_attr(Zuka()->settings()->get('single_ajax_add_cart', 'off')),
            'i18n' => array(
                'backtext' => esc_attr_x('Back', 'front-view', 'zuka'),
                'compare' => array(
                    'view' => esc_attr_x('View List Compare', 'front-view', 'zuka'),
                    'success' => esc_attr_x('has been added to comparison list.', 'front-view', 'zuka'),
                    'error' => esc_attr_x('An error occurred ,Please try again !', 'front-view', 'zuka')
                ),
                'wishlist' => array(
                    'view' => esc_attr_x('View List Wishlist', 'front-view', 'zuka'),
                    'success' => esc_attr_x('has been added to your wishlist.', 'front-view', 'zuka'),
                    'error' => esc_attr_x('An error occurred, Please try again !', 'front-view', 'zuka')
                ),
                'addcart' => array(
                    'view' => esc_attr_x('View Cart', 'front-view', 'zuka'),
                    'success' => esc_attr_x('has been added to your cart', 'front-view', 'zuka'),
                    'error' => esc_attr_x('An error occurred, Please try again !', 'front-view', 'zuka')
                ),
                'global' => array(
                    'error' => esc_attr_x('An error occurred ,Please try again !', 'front-view', 'zuka'),
                    'comment_author' => esc_attr_x('Please enter Name !', 'front-view', 'zuka'),
                    'comment_email' => esc_attr_x('Please enter Email Address !', 'front-view', 'zuka'),
                    'comment_rating' => esc_attr_x('Please select a rating !', 'front-view', 'zuka'),
                    'comment_content' => esc_attr_x('Please enter Comment !', 'front-view', 'zuka'),
                    'continue_shopping' => esc_attr_x('Continue Shopping', 'front-view', 'zuka'),
                    'cookie_disabled' => esc_attr_x('We are sorry, but this feature is available only if cookies are enabled on your browser', 'front-view', 'zuka')
                )
            ),
            'popup' => array(
                'max_width' => esc_attr(Zuka()->settings()->get('popup_max_width', 790)),
                'max_height' => esc_attr(Zuka()->settings()->get('popup_max_height', 430))
            ),
            'js_path'       => esc_attr(Zuka::$template_dir_url . '/assets/js/plugins/' . $script_min_path),
            'theme_path'    => esc_attr(Zuka::$template_dir_url . '/'),
            'ajax_url'      => esc_attr(admin_url('admin-ajax.php')),
            'mm_mb_effect' => esc_attr(Zuka()->settings()->get('mm_mb_effect', 1)),
            'header_height' => array(
                'desktop' => array(
                    'normal' => esc_attr(str_replace('px', '', Zuka()->settings()->get('header_height', 100))),
                    'sticky' => esc_attr(str_replace('px', '', Zuka()->settings()->get('header_sticky_height', 80)))
                ),
                'tablet' => array(
                    'normal' => esc_attr(str_replace('px', '', Zuka()->settings()->get('header_sm_height', 100))),
                    'sticky' => esc_attr(str_replace('px', '', Zuka()->settings()->get('header_sm_sticky_height', 80)))
                ),
                'mobile' => array(
                    'normal' => esc_attr(str_replace('px', '', Zuka()->settings()->get('header_mb_height', 100))),
                    'sticky' => esc_attr(str_replace('px', '', Zuka()->settings()->get('header_mb_sticky_height', 80)))
                )
            ),
            ...
            'mobile_bar' => esc_attr(Zuka()->settings()->get('enable_header_mb_footer_bar_sticky', 'always'))
        )));
        ...
        wp_add_inline_style('zuka-theme', Zuka_Helper::compress_text($this->dynamic_css(), true));
    ...
    {
        ob_start();
        include Zuka::$template_dir_path . '/framework/functions/additional_css.php';
        include Zuka::$template_dir_path . '/framework/functions/dynamic_css.php';
        return ob_get_clean();
    }

    public function get_custom_css_from_setting(){
        if( $la_custom_css = Zuka()->settings()->get('la_custom_css') ){
            printf( '<%1$s id="zuka-extra-custom-css">%2$s</%1$s>', 'style', $la_custom_css);
        }
    }
    ...
    */
    ...
    {
        $handlers[] = 'zuka-modernizr-custom';
        $handlers[] = 'zuka-plugins';
        $handlers[] = 'zuka-theme';
        return $handlers;
    }
    ...
    {
        $config = array();
        $metadata = Zuka()->settings()->get_post_meta(get_the_ID());
    ...
    public function get_gfont_from_setting(){
        $array = array();
        $main_font = Zuka()->settings()->get('main_font');
        $secondary_font = Zuka()->settings()->get('secondary_font');
        $highlight_font = Zuka()->settings()->get('highlight_font');
    ...
    public function get_google_font_url(){
        ...
        $main_font = (array) Zuka()->settings()->get('main_font');
        $secondary_font = (array) Zuka()->settings()->get('secondary_font');
        $highlight_font = (array) Zuka()->settings()->get('highlight_font');
    ...
    public function get_google_font_code_url() {
        $fonts_url = '';
        $_font_code = Zuka()->settings()->get('font_google_code', '');
        if(!empty($_font_code)){
    ...
    public function get_google_font_typekit_url(){
        $fonts_url = '';
        $_api_key = Zuka()->settings()->get('font_typekit_kit_id', '');
        if(!empty($_api_key)){
    ...
    public function add_custom_header_js(){
        printf( '<%1$s>try{ %2$s }catch (ex){}</%1$s>', 'script', Zuka()->settings()->get('header_js') );
    }

    public function add_custom_footer_js(){
        printf( '<%1$s>try{ %2$s }catch (ex){}</%1$s>', 'script', Zuka()->settings()->get('footer_js') );
    }

    public function add_meta_into_head(){
        do_action('zuka/action/head');
    }

    public function override_style_from_builder(){
        add_action('wp_enqueue_scripts', function(){
            wp_enqueue_style( 'lahfb-frontend-styles', Zuka::$template_dir_url . '/assets/css/header-builder.css', array(), '1.0' );    
        });
    }
}