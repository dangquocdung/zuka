...
class Zuka_Autoload {
    ...
    public function __construct( $args = array() ) {
        ...
        if ( empty( self::$transient_name ) ) {
            self::$transient_name = 'zuka_autoloader_paths_' . md5( __FILE__ );
        }
    ...
    protected function get_path( $class_name ) {
        ...
        $paths = array();
        if ( 0 === stripos( $class_name, 'Zuka' ) ) {
            $path     = wp_normalize_path( Zuka::$template_dir_path . '/framework/classes/' );

            $class_name = str_replace( array( 'Zuka_' ), '', $class_name );
