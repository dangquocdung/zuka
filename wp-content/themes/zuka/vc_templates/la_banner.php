...
<div id="<?php echo esc_attr($unique_id)?>" class="<?php echo esc_attr($css_class);?>">
    <div class="box-inner">
        ...
        zuka_deactive_filter( 'jetpack_photon_override_image_downsize', '__return_true' );
        ...
        <div class="banner--info">
            <?php
                echo zuka_render_variable($title_1_html . $title_2_html . $title_3_html);
                if($use_link){
                    if(empty($a_title)){
                        echo sprintf(
                            '<a class="banner--btn hidden" %s>%s</a>',
                            $a_attributes,
                            esc_html($a_title)
                        );
                    }
    ...
    <?php echo '<style>'; ?>
        ...
        .<?php echo esc_attr($unique_id)?>.la-banner-box .banner--image:after{
            background-color: <?php echo zuka_render_variable($overlay_bg_color) ?>;
        }
        <?php endif; ?>
        <?php if(!empty($overlay_hover_bg_color)): ?>
        .<?php echo esc_attr($unique_id)?>.la-banner-box:hover .banner--image:after{
            background-color: <?php echo zuka_render_variable($overlay_hover_bg_color) ?>;
        }
        <?php endif; ?>
    <?php echo '</style>'; ?>
    <?php endif; ?>
