...
$active_shop_masonry = Zuka()->settings()->get('active_shop_masonry', 'off');
$shop_masonry_column_type = Zuka()->settings()->get('shop_masonry_column_type', 'default');
$woocommerce_shop_masonry_columns = Zuka()->settings()->get('woocommerce_shop_masonry_columns');

$product_masonry_container_width = Zuka()->settings()->get('product_masonry_container_width', 1170);
$product_masonry_item_width = Zuka()->settings()->get('product_masonry_item_width', 270);
$product_masonry_item_height = Zuka()->settings()->get('product_masonry_item_height', 450);
$woocommerce_shop_masonry_custom_columns = Zuka()->settings()->get('woocommerce_shop_masonry_custom_columns');
$shop_masonry_item_setting = Zuka()->settings()->get('shop_masonry_item_setting');


$woocommerce_pagination_type = Zuka()->settings()->get('woocommerce_pagination_type', 'pagination');

$item_gap = Zuka()->settings()->get('shop_item_space', 'default');

if($item_gap == 'zero'){
    $item_gap = 0;
}

$column_tmp = Zuka()->settings()->get('woocommerce_shop_page_columns');

if($is_main_loop && zuka_string_to_bool($active_shop_masonry)){
...
if($is_main_loop && zuka_string_to_bool($active_shop_masonry) && $shop_masonry_column_type == 'custom'){
...
if($is_main_loop && zuka_string_to_bool($active_shop_masonry)){
    zuka_set_wc_loop_prop('prods_masonry', true);
    ...
    if( $shop_masonry_column_type != 'custom' ){
        ...
        $loopCssClass[] = zuka_render_grid_css_class_from_columns($columns);
    }
    else{
        ...
        if(!empty($shop_masonry_item_setting) && is_array($shop_masonry_item_setting)){
            foreach($shop_masonry_item_setting as $k => $size){
                ...
                if(!empty($size['image_size'])){
                    $__new_item_sizes[$k]['s'] = Zuka_Helper::get_image_size_from_string($size['image_size']);
                }
            }
        }
        zuka_set_wc_loop_prop('item_sizes', $__new_item_sizes);
        $masonry_component_type[] = 'AdvancedMasonry';
    }
    zuka_set_wc_loop_prop('image_size', Zuka_Helper::get_image_size_from_string(Zuka()->settings()->get('product_masonry_image_size', 'shop_catalog')));
    ?>
<?php
}
else{
