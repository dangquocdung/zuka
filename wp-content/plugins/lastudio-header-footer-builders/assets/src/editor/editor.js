    ...
    $(document).ready(function () {
        ...
        $(window).on('load', function () {
            ...
            function lahfbIsFrontendBuilder() {
                if ($body.hasClass('lahfb-frontend-builder-wrap')) {
                    return true;
                }
                return false;
            }
            ...
            $wrap.find('.lahfb-modal-wrap').on('click', '.lahfb-elements-item a', function (event) {
                ...
                let controlsHtml = `<span class="lahfb-controls">
                ...
                if ($this.closest('.lahfb-elements-item').hasClass('lahfb-clipboard-item')) {
                    ...
                    let elemID = elemName + '_' + elemUniqueID;
                    let newElem = elemName + '_' + uniqueID;
                        ...
                        if(data.success){
                            ...
                            for ( let key in data.data ){
                                ...
                                if(typeof obj.image !== "undefined" ){
                                    __html += '<a class="lahfb-prebuild-item" data-saved-name="'+key+'" href="#"><img src="'+obj.image+'" alt="'+obj.name+'"/></a>'
                                }
                                else{
                                    __html += '<a class="lahfb-prebuild-item" data-saved-name="'+key+'" href="#">'+obj.name+'</a>'
                                }
                            }
                            $('.lahfb-predefined-modal-contents').html(__html);
            ...
            $wrap.on('click', '.lahfb-edit-btn', function (event) {
                ...
                lahfbElementSettings($currentElement, currentPanel);
            });
            ...
            $wrap.on('click', '.lahfb-hide-btn', function (event) {
                ...
                if (elemName == 'header-area' || elemName == 'sticky-area') {
                    ...
                    if (mustBeHidden) {
                        $this.removeClass('fa fa-eye').addClass('fa fa-eyeslash');
             ...
             * @version    1.0.0
             */
            function lahfbElementSettings($currentElement, currentPanel, new_el = false) {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        $('.la-header-builder-wrap').append(data);
            ...
            function lahfbFields() {
                ...
                $modalWrap.find('.lahfb-field').find('.lahfb-field-input').each(function () {
                    ...
                    if ($fieldInput.hasClass('lahfb-icon-field')) {
                        ...
                        if (value) {
                            $icon.removeClass('lahfb-active');
                            $iconsWrap.find('.la-icon-wrap').find('label[for="' + value + '"]').addClass('lahfb-active');
                        } else {
                            $icon.removeClass('lahfb-active');
                        }
                    }
            ...
            $('.lahfb-action-collapse').on('click', function () {
                let $elem = $(this);
                let $editor = $('.la-header-builder-wrap.lahfb-frontend-builder').css('bottom', '-60%');
            ...
            function lahfbLiveDynamic() {
                ...
                $.ajax({
                    ...
                    data: {
                        action: 'lahfb_ajax_action',
                        router: 'live_dynamic_styles',
                        nonce: lahfb_localize.nonce,
                        headerPreset: lahfb_localize.prebuild_header_key
                    },
                    success: function (data) {
                        let $dynamic_inline = $LaStudo_Iframe.contents().find('#lahfb-frontend-styles-inline-css');
                        $dynamic_inline[0].innerHTML = data;
                    }
                });
            }
            if(lahfbIsFrontendBuilder()){
            ...
            function lahfbSaveAllData() {
                ...
                if($LaStudo_Iframe.length){
                    $LaStudoIframeFrontend = $LaStudo_Iframe.contents().find('#lastudio-header-builder');
                    if ($LaStudoIframeFrontend.children('.lahfb-spinner-wrap').length == 0) {
                        $LaStudoIframeFrontend.prepend('<div class="lahfb-spinner-wrap"><div class="lahfb-spinner"><div class="double-bounce1"></div><div class="double-bounce2"></div></div></div>');
                    }
                }
                ...
                $.ajax({
                        ...
                        success: function (data) {
                            if(lahfbIsFrontendBuilder()){
                                $LaStudoIframeFrontend[0].outerHTML = data;
                            }
                        },
                        complete: function () {
                    ...
                    for (let rowKey in frontendComponents[screen_key]) {
                        for (let cell_key in frontendComponents[screen_key][rowKey]) {
                            ...
                            if (cell_key == 'settings' || cell_key == 'left_settings' || cell_key == 'center_settings' || cell_key == 'right_settings') {
                                ...
                                let elemID = cell.uniqueId;
                                let component_key = elemName + '_' + elemID;
                                ...
                                if (cell.length != 0) {
                                    ...
                                    for (let el of cell) {
                                        let elemName = el.name;
                                        let elemID = el.uniqueId;
                                        let component_key = elemName + '_' + elemID;
