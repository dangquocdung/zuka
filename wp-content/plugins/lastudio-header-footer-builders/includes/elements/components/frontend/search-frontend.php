...
function lahfb_search( $atts, $uniqid, $once_run_flag ) {
    ...
    $extra_html .= '<button class="search-button" type="submit"><i class="'.esc_attr(lahfb_rename_icon($search_icon)).'"></i></button>';

    // login
    $placeholder_text 	= ! empty( $placeholder_text ) ? LAHFB_Helper::render_string($placeholder_text) : esc_html__( 'Search' , 'lahfb-textdomain' );
