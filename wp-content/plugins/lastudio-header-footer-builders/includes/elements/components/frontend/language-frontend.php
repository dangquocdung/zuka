...
function lahfb_language( $atts, $uniqid, $once_run_flag ) {
	...
	if ( $once_run_flag ) :
		...
		$dynamic_style = '';
        $dynamic_style .= lahfb_styling_tab_output( $atts, 'Typography', '#lastudio-header-builder .language_switcher_' . esc_attr( $uniqid ) .' a' );
        $dynamic_style .= lahfb_styling_tab_output( $atts, 'Background', '#lastudio-header-builder .language_switcher_' . esc_attr( $uniqid ) );
        $dynamic_style .= lahfb_styling_tab_output( $atts, 'Box', '#lastudio-header-builder .language_switcher_' . esc_attr( $uniqid ) );
	...
	// render
	$out .= '
	<div class="lahfb-element lahfb-icon-wrap lahfb-language ' . esc_attr( $extra_class ) . ' language_switcher_'.esc_attr( $uniqid ).'">';
