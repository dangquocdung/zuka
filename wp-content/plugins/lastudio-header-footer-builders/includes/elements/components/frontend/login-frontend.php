...
function lahfb_login($atts, $uniqid, $once_run_flag)
{
    ...
    if (is_user_logged_in()) {
        ...
        $login_text = $show_avatar . '<span class="lahfb-login-text-modal">' . $login_text . '</span>';
    }
    ...
    if ($open_form == 'modal') {
        $out .= '<a class="lahfb-modal-element lahfb-modal-target-link" href="#w-login" data-effect="mfp-zoom-in"></a>';
    }
    ...
    if ($login_type == 'text' || $login_type == 'icon_text') {
        ...
        $out .= '<i class="fa fa-user-circle-o"></i>';
    }
    $out .= '</div>';

    if($once_run_flag) {
        if ($open_form == 'modal') {
            $out .= '<div id="w-login" class="w-login modal-login white-popup mfp-with-anim mfp-hide' . (!is_user_logged_in() ? ' show-login-form' : ' show-profile-form') . '">';
        } elseif ($open_form == 'dropdown') {
            $out .= '<div class="lahfb-trigger-element js--trigger-login-dropdown-icon"></div><div id="w-login" class="w-login la-element-dropdown">';
        }
        ob_start();
