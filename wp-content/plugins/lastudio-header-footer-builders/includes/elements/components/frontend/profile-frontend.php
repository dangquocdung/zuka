...
function lahfb_profile( $atts, $uniqid, $once_run_flag ) {
	...
	if ( $once_run_flag ) :
		...
		$dynamic_style = '';
        $dynamic_style .= lahfb_styling_tab_output( $atts, 'Image', '#lastudio-header-builder .com_profile_' . esc_attr( $uniqid ) . ' .lahfb-profile-image-wrap' );
        $dynamic_style .= lahfb_styling_tab_output( $atts, 'Name', '#lastudio-header-builder .com_profile_' . esc_attr( $uniqid ) . ' .lahfb-profile-name' );
        $dynamic_style .= lahfb_styling_tab_output( $atts, 'Socials Text', '#lastudio-header-builder .com_profile_' . esc_attr( $uniqid ) . ' .lahfb-profile-socials-icons a' );
        $dynamic_style .= lahfb_styling_tab_output( $atts, 'Socials Box', '#lastudio-header-builder .com_profile_' . esc_attr( $uniqid ) . ' .lahfb-profile-socials-icons' );
        $dynamic_style .= lahfb_styling_tab_output( $atts, 'Box', '#lastudio-header-builder .com_profile_' . esc_attr( $uniqid ) );
	...
	// render
	$out .= '<div class="lahfb-element lahfb-element-wrap lahfb-profile-wrap lahfb-profile' . esc_attr( $extra_class ) . ' com_profile_'.esc_attr( $uniqid ).'">';
		...
		if ( $socials == 'true' ) {
			$out .= '<div class="lahfb-profile-socials-wrap">
						<div class="lahfb-profile-socials-text-wrap">
							...
							<div class="lahfb-profile-socials-text">' . esc_html__( 'SOCIALS', 'lahfb-textdomain' ) . ' <i class="fa fa-angle-down"></i>
