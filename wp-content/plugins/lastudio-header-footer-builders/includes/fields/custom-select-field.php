...
function lahfb_custom_select( $settings ) {
	...
	if ( $options ) :
		...
		$option .= '<span>' . esc_html__( 'Empty', 'lahfb-textdomain' ) . '</span>';
	endif;
	...
	$output = '
		<div class="lahfb-field w-col-sm-12">
			...
			<div class="lahfb-custom-select wp-clearfix">
				...
				<input type="text" class="lahfb-field-input lahfb-field-custom-select" data-field-name="' . esc_attr( $id ) . '" data-field-std="' . esc_attr( $default ) . '" placeholder="' . esc_html__( 'Custom', 'lahfb-textdomain' ) . '">
			</div>
		</div>
	';
